...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We use a safe parse because if the server crashes (500), it might not return JSON.
      const data = await res.json().catch(() => null);

      // 4. Handle Errors based on Status Code
      if (!res.ok) {
        // Case A: 401 Unauthorized (Wrong Password/User)
        if (res.status === 401) {
          throw new Error("Invalid phone number or password");
        }
        // Case B: 500 Server Error (Backend Crash)
        if (res.status === 500) {
          throw new Error("Invalid phone number or password");
        }
        // Case C: Other Errors (Use message from backend if available)
      ...
      // Display the clean error message we threw above
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-midnight via-midnight-light to-midnight-lighter text-white font-sans flex flex-col">
      {/* Header */}
      <header className="flex py-2 justify-between items-center px-6 border-b border-gold/10 bg-midnight/50 backdrop-blur-md">
        <Link href="/">
          <Image
            src={LOGO_URL}
            alt="Zuugnu"
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="bg-linear-to-r from-midnight-light to-midnight border border-gold/30 rounded-2xl p-6 text-center shadow-[0_0_20px_rgba(255,215,0,0.15)] relative overflow-hidden group">
            ...
            <p className="text-gray-300 text-sm leading-relaxed">
              WhatsApp your{" "}
              <span className="text-gold font-semibold">Full Name</span> to{" "}
              <br />
              <a
                href="https://wa.me/918800607598"
                className="text-xl font-bold text-teal hover:underline decoration-dotted decoration-teal underline-offset-4"
              >
                +91 88006 07598
              </a>
              <br />
      ...
      <footer className="py-6 text-center text-xs text-gray-500 border-t border-white/5">
        <p>
          &copy; 2025 Zuugnu.com <span className="mx-2 opacity-30">||</span> All
          rights reserved.
        </p>
      </footer>
    </div>
